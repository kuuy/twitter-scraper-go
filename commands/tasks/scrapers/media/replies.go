    ...
    Subcommands: []*cli.Command{
      ...
      },
      {
        ...
        Action: func(c *cli.Context) (err error) {
          id := c.Args().Get(0)
          if id == "" {
            log.Fatal("twitter replies id can not be empty")
            return nil
          }
...
func (h *RepliesHandler) Init(limit int) error {
  log.Println(fmt.Sprintf("twitter replies media tasks init..."))
  conditions := map[string]interface{}{}
  replies := h.RepliesRepository.Ranking(
    []string{"id", "timestamp"},
    conditions,
    "timestamp",
    -1,
  ...
  for _, reply := range replies {
    name := fmt.Sprintf("%v@media.replies", reply.ID)
...
func (h *RepliesHandler) Apply(id string) (err error) {
  log.Println(fmt.Sprintf("tasks media replies apply..."))
  reply, err := h.RepliesRepository.Find(id)
  if errors.Is(err, gorm.ErrRecordNotFound) {
    return
  }
  name := fmt.Sprintf("%v@media", reply.ID)
  action := config.TASK_ACTION_SCRAPERS_MEDIA_REPLIES
  params := map[string]interface{}{
    "id": reply.ID,
  }
...
func (h *RepliesHandler) Process(limit int) error {
  log.Println(fmt.Sprintf("tasks media replies processing..."))
  ...
  for _, task := range tasks {
    mutex := common.NewMutex(
      h.Rdb,
      h.Ctx,
      fmt.Sprintf(config.LOCKS_TASKS_SCRAPERS_MEDIA_REPLIES_PROCESS, task.ID),
    )
